...
def load_events_h5(path):
    ...
    if 'x' in file.keys():
        ...
        length = len(file['events_x'])
        events = np.zeros([length, 4], dtype=np.float32)
        events[:, 0] = file['events_x']
        events[:, 1] = file['events_y']
        events[:, 2] = file['events_t']
        events[:, 3] = file['events_p']
    file.close()
    return events


def load_events_h5_seq(path, img_idxs):
    file = h5py.File(path, 'r')
    raw_idxs = np.array(file['idx'])
    raw_idxs = np.append(raw_idxs, len(file['x']))
    seq_events = []
    for i in range(len(img_idxs) - 1):
        start_raw_idx = raw_idxs[img_idxs[i]-1]
        end_raw_idx = raw_idxs[img_idxs[i+1]-1]
        events = np.zeros([end_raw_idx-start_raw_idx, 4], dtype=np.float32)
        events[:, 0] = file['x'][start_raw_idx:end_raw_idx]
        events[:, 1] = file['y'][start_raw_idx:end_raw_idx]
        events[:, 2] = file['t'][start_raw_idx:end_raw_idx]
        events[:, 3] = file['p'][start_raw_idx:end_raw_idx]
...
def eventsToVoxel(events, num_bins=5, height=None, width=None, event_polarity=False, temporal_bilinear=True):
    return eventsToVoxelTorch(events, num_bins, height, width, event_polarity, temporal_bilinear).numpy().transpose(1, 2, 0)
...
def eventsToColorImage(events, size=None, background='white'):
    """
    ...
    if isinstance(background, str) and background == 'black':
        dvs_img = np.zeros((height, width, 3), dtype=np.uint8)
    elif isinstance(background, str) and background == 'white':
        dvs_img = np.ones((height, width, 3), dtype=np.uint8) * 255
    elif isinstance(background, np.ndarray):
        dvs_img = background.copy()
    else:
        raise NotImplementedError(background)

    dvs_img[ey[ep==ep.max()], ex[ep==ep.max()]] = [255, 0, 0]
    dvs_img[ey[ep==ep.min()], ex[ep==ep.min()]] = [0, 0, 255]

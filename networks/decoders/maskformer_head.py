# from https://github.com/open-mmlab/mmdetection/tree/main/mmdet/models/dense_heads/maskformer_head.py
...
class MaskFormerHead(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_bbox: ConfigType = dict(
                     type='L1Loss',
                     loss_weight=1.0),  # bbox loss config
        ...
        self.bbox_embedding_layer = nn.Sequential(
    ...
    def preprocess_gt(
        ...
        batch_gt_instances = [
            InstanceData(labels=label, masks=mask, bboxes=bbox)  
            for label, mask, bbox in zip(labels, masks, gt_bboxes_list)
        ]
    ...
    def get_targets(
        ...
        results = multi_apply(self._get_targets_single, cls_scores_list,
                              mask_preds_list, bbox_preds_list,
                              batch_gt_instances, batch_img_metas)
        (labels_list, label_weights_list, mask_targets_list, mask_weights_list, bbox_targets_list,
         bbox_weights_list, pos_inds_list, neg_inds_list, sampling_results_list) = results[:9]
        ...
        res = (labels_list, label_weights_list, mask_targets_list,
               mask_weights_list, bbox_targets_list, bbox_weights_list, avg_factor)
    ...
    def _get_targets_single(self, cls_score: Tensor, mask_pred: Tensor,
                            bbox_pred: Tensor, gt_instances: InstanceData,
        ...
        gt_bboxes = gt_instances.bboxes  # GT bbox
        ...
        pred_instances = InstanceData(scores=cls_score, masks=mask_pred, bboxes=bbox_pred)
        downsampled_gt_instances = InstanceData(
            labels=gt_labels, masks=gt_masks_downsampled, bboxes=gt_bboxes)
        # assign and sample
        ...
        # bbox target
        bbox_targets = bbox_pred.new_zeros((self.num_queries, 4))
        bbox_weights = bbox_pred.new_zeros((self.num_queries, ))
        if len(pos_inds) > 0:
            bbox_targets[pos_inds] = gt_bboxes[sampling_result.pos_assigned_gt_inds]
            bbox_weights[pos_inds] = 1.0

        return (labels, label_weights, mask_targets, mask_weights, bbox_targets, bbox_weights, pos_inds,
                neg_inds, sampling_result)

    def loss_by_feat(self, all_cls_scores: Tensor, all_mask_preds: Tensor, all_bbox_preds: Tensor,
        ...
        ] # b, labelshape -> num_decoder, b, labelshape; b, maskshape -> num_decoder, b, maskshape
        img_metas_list = [batch_img_metas for _ in range(num_dec_layers)]
        losses_cls, losses_mask, losses_dice, losses_bbox = multi_apply(
            self._loss_by_feat_single, all_cls_scores, all_mask_preds, all_bbox_preds,
            batch_gt_instances_list, img_metas_list)

        loss_dict = dict()
        loss_dict['loss_cls'] = sum(losses_cls)
        loss_dict['loss_mask'] = sum(losses_mask)
        loss_dict['loss_dice'] = sum(losses_dice)
        loss_dict['loss_bbox'] = sum(losses_bbox)   # bbox loss

        return loss_dict

    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor, bbox_preds: Tensor,
                             batch_gt_instances: List[InstanceData],
        ...
        (labels_list, label_weights_list, mask_targets_list, mask_weights_list, 
         bbox_targets_list, bbox_weights_list, avg_factor) = self.get_targets(cls_scores_list, mask_preds_list, bbox_preds_list,
                                        batch_gt_instances, batch_img_metas)
        ...
        # Bbox loss
        # compute global avg_factor
        num_total_bboxes = reduce_mean(cls_scores.new_tensor([avg_factor]))
        num_total_bboxes = max(num_total_bboxes, 1)
        ...
        bbox_weights = bbox_weights.flatten(0, 1)

        valid_mask = ~(bbox_targets == torch.tensor([-1, -1, -1, -1], dtype=torch.float32, device=bbox_targets.device)).all(dim=1)
        bbox_targets = bbox_targets[valid_mask]
        bbox_preds = bbox_preds[valid_mask]

        bbox_weights = bbox_weights[valid_mask].unsqueeze(-1)
        if self.loss_bbox is not None:
            loss_bbox = self.loss_bbox(
                bbox_preds, bbox_targets, bbox_weights, avg_factor=num_total_bboxes)
        else:
            loss_bbox = 0


        return loss_cls, loss_mask, loss_dice, loss_bbox

    def forward(self, x: Tuple[Tensor], batch_img_metas: List[dict]) -> Tuple[Tensor]:
        ...
        # bbox_preds extracted from out_dec
        bbox_embedding = self.bbox_embedding_layer(out_dec)
        all_bbox_preds = self.bbox_coord_head(bbox_embedding) 
        ...
        return all_cls_scores, all_mask_preds, all_bbox_preds
    ...
    def loss(
        ...
        # forward
        all_cls_scores, all_mask_preds, all_bbox_preds = self(x, batch_data_samples)
        ...
        # loss
        losses = self.loss_by_feat(all_cls_scores, all_mask_preds, all_bbox_preds,
                                   batch_gt_instances, batch_img_metas)
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        bbox_preds_results = all_bbox_preds[-1]
        ...
        return mask_cls_results, mask_pred_results, bbox_preds_results

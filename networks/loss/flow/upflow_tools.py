# from https://github.com/coolbeam/UPFlow_pytorch/blob/main/utils/tools.py
...
class tools():
    ...
    class boundary_dilated_warp():
        ...
        @classmethod
        def get_grid(cls, batch_size, H, W, start=None):
            ...
            # print("start", start)
            # grid[:, :2, :, :] = grid[:, :2, :, :] + start  # add patch offset within original image
        ...
        def transformer(cls, I, vgrid, train=True):
            ...
            def _interpolate(im, x, y, out_size, scale_h):
                ...
                base = _repeat(torch.arange(0, num_batch) * dim1, out_height * out_width)  # simply mark the index position of each image in batch
                # base = torch.arange(0,num_batch) * dim1
                # base = base.reshape(-1, 1).repeat(1, out_height * out_width).reshape(-1).int()
                # difference? expand does not copy data .reshape(-1,1).expand(-1,out_height * out_width).reshape(-1)
                if torch.cuda.is_available():
                    dim2 = dim2.cuda()
                    dim1 = dim1.cuda()
                    y0 = y0.cuda()
        ...
        @classmethod
        def warp_im(cls, I_nchw, flow_nchw, start_n211=None):
...
class network_tools():
    ...
    def normalize_features(cls, feature_list, normalize, center, moments_across_channels=True, moments_across_images=True):
        ...
        Args:
          ...
          moments_across_channels: bool flag, compute mean and std across channels, UFlow default is True
          moments_across_images: bool flag, compute mean and std across images, UFlow default is True
    ...
    def flow_smooth_delta(cls, flow, if_second_order=False):
        ...
        # smooth_loss = dx.abs().mean() + dy.abs().mean()  # + dx2.abs().mean() + dxdy.abs().mean() + dydx.abs().mean() + dy2.abs().mean()
        # temporarily disable second-order smoothness loss, seems too strong and prevents photo loss reduction TODO
        return smooth_loss
